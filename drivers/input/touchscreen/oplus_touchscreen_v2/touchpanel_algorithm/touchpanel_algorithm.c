			 ...
			 uint16_t delta_time, struct algo_kalman_info *kal_value)
{
	...
	if (temp > kal_value->kalman_dis) {
		q_num = kal_value->Q_Max;
		r_num = KALMAN_LEVEL - q_num;
		...
		if (kal_lv == 0) {
			q_num = kal_value->Q_Min;
			r_num = KALMAN_LEVEL - kal_value->Q_Min;

		} else {
			q_num = kal_value->Q_Min + (2 * temp * kal_lv + filter_dis) / (2 * filter_dis);
			r_num = KALMAN_LEVEL - q_num;
		}
	}
	...
	x_now = (x_now + (long)(p_mid * (ResrcData - x_mid)) / (p_mid + r_num)) /
		VARIABLE_GAIN;

	p_now = (2 * r_num * p_mid + p_mid + r_num) / (2 * (p_mid + r_num));

	*p_last = (uint16_t)p_now;

	*x_last = (uint16_t)x_now;
	TPD_DEBUG("R %d,Q %d, p_mid %d, p_now %d\n", r_num, q_num, p_mid, p_now);
}
...
static void end_stretch_handle(struct touchpanel_data *ts,
			       struct point_info *points)
{
	struct touch_algorithm_info *algo_info = ts->algo_info;
	int i = 0;
	int area = algo_info->end_stretch.end_area;
	int limit_time = algo_info->end_stretch.limit_time;
	int limit_speed = algo_info->end_stretch.limit_speed;
	int ratio = algo_info->end_stretch.stretch_ratio;
	int speed;
	int max_x = ts->resolution_info.max_x - 1;
	int count = 0;
	int delta_y;

	if (algo_info->phone_direction != BOTTOM_SIDE) {
		for (i = 0; i < ts->max_num; i++) {
			algo_info->last_status[i] = 0;
		}
		return;
	}
	for (i = 0; i < ts->max_num; i++) {
		struct algo_point *last_point = &algo_info->last_points[i];
		struct algo_point *last_last_point = &algo_info->last_last_points[i];
		if (algo_info->touch_status[i] != 0) {
			if (algo_info->last_status[i] == 0) {
				last_point->x = points[i].x;
				last_point->y = points[i].y;

				last_last_point->x = points[i].x;
				last_last_point->y = points[i].y;
			}
		} else if (algo_info->last_status[i] != 0) {
			if (algo_info->point_buf[i].touch_time > limit_time) {
				continue;
			}
			delta_y = abs(last_point->y - last_last_point->y);
			if (last_point->x < area) {
				if (last_point->x + limit_speed > last_last_point->x) {
					continue;
				}
				speed = last_last_point->x - last_point->x;
				if (speed <= delta_y) {
					continue;
				}
				if (last_point->x < speed) {
					continue;
				}
				speed = speed + ratio*speed/16;
				/* need stretch */

				algo_info->last_status[i] = 0x80;
				count++;
				if (last_point->x < speed) {
					last_point->x = 0;
				} else {
			...
			if (last_point->x + area > max_x) {
				if (last_last_point->x + limit_speed > last_point->x) {
					continue;
				}
				speed = last_point->x - last_last_point->x;
				if (speed <= delta_y) {
					continue;
				}
				if (last_point->x + speed > max_x) {
					continue;
				}
				speed = speed + ratio*speed/16;
				/* need stretch */
	...
	if (count) {
		...
		for (i = 0; i < ts->max_num; i++) {
			struct algo_point *last_point = &algo_info->last_points[i];
			if (0x80 == algo_info->last_status[i]) {
				input_mt_slot(ts->input_dev, i);
				input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 1);
				input_report_key(ts->input_dev, BTN_TOUCH, 1);
				input_report_key(ts->input_dev, BTN_TOOL_FINGER, 1);
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, last_point->x);
				input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, last_point->y);
				TPD_INFO("end stretch id %d, %d,%d", i, last_point->x, last_point->y);
				algo_info->last_status[i] = 0;
			}
		}
		input_sync(ts->input_dev);
	...
	for (i = 0; i < ts->max_num; i++) {
		...
		last_last_point->x = last_point->x;
		last_last_point->y = last_point->y;

		last_point->x = points[i].x;
		last_point->y = points[i].y;

		algo_info->last_status[i] = algo_info->touch_status[i];
	}
}
			   ...
			   struct point_info *points)
{
	...
	if (algo_info->end_stretch.end_area > 0) {
		end_stretch_handle(ts, points);
	}
...
void release_algorithm_points(struct touchpanel_data *ts)
{
	...
	for (i = 0; i < ts->max_num; i++) {
		...
		ts->algo_info->touch_status[i] = 0;
		ts->algo_info->last_status[i] = 0;
	}
...
EXPORT_SYMBOL(set_algorithm_direction);
...
void oplus_touch_algorithm_init(struct touchpanel_data *ts)
{
	...
	rc = of_property_read_u32_array(ts->dev->of_node, "end_stretch_value",
					(uint32_t *)&algo_info->end_stretch,
					sizeof(struct algo_end_stretch) / sizeof(uint32_t));

	if (rc < 0) {
		TPD_INFO("error:end_stretch_value kalman value not setting in dts!");
		algo_info->end_stretch.end_area = 0;
		algo_info->end_stretch.limit_time = 0;
	}
	TPD_INFO("End stretch value is %d,%d,%d,%d.\n",
		 algo_info->end_stretch.end_area,
		 algo_info->end_stretch.limit_time,
		 algo_info->end_stretch.limit_speed,
		 algo_info->end_stretch.stretch_ratio);
	...
	algo_info->last_points = kzalloc(sizeof(struct algo_point) * ts->max_num,
					 GFP_KERNEL);
	if (algo_info->last_points == NULL) {
		err = err | 0x02;
		goto OUT;
	}

	algo_info->last_last_points = kzalloc(sizeof(struct algo_point) * ts->max_num,
					      GFP_KERNEL);
	if (algo_info->last_last_points == NULL) {
		err = err | 0x02;
		goto OUT;
	}

	algo_info->touch_status = kzalloc(ts->max_num, GFP_KERNEL);
	if (algo_info->touch_status == NULL) {
		err = err | 0x02;
		goto OUT;
	}

	algo_info->last_status = kzalloc(ts->max_num, GFP_KERNEL);
	if (algo_info->last_status == NULL) {
		err = err | 0x02;
		goto OUT;
	}
